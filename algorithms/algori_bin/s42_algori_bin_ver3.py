...
multi_percent2 = 1   # 两个货物余量空间的使用率
multi_percent3 = 0.8    # 多个个货物余量空间的使用率
lw_mmm_cm = 2   ### 长或者宽，其中最大余量必须大于这么多
...
class OneContainerOneItemSolution():
    ...
    def __init__(self, goods_dict):
        ...
        f_path = os.path.join(config.f_path['data_json'], 'bin_special_rule_dict.json')   # 路由的字典
        self.bin_special_rule_dict = json.load(open(f_path, "rb"))
        ...
        self.thermometer_shape = '12*10*3'  # 温度计的尺寸（厘米） 
        self.shape_thermometer_num = {}

        self.rule_accept_bins = []
        self.min_margin_cm = 2  # 默认为2cm的最小边距


        if __name__=='__main__':
            if os.path.exists('test_output') is False:os.mkdir('test_output')

    def check_str_list_in(self, father_str, sons_list):
        state = True
        for s in sons_list:
    ...
    def check_goods_str(self):
        ...
        # "cu_id": "202000722",  # 客户编号
        # "cp_id": "-1",          # 项目编号
        # "goods_name": "人类EGFR突变基因检测试剂盒（多重荧光PCR法）",  # 货物名称

        key_list2 = 'cu_id,cp_id,goods_name'.split(',') 
        for i in key_list2:
            if i not in self.goods_dict:
                self.goods_dict[i] = "-1"
            else:
                self.goods_dict[i] = str(self.goods_dict[i])  # 都变成字符串

        key_list3 = 'thermometer_num'.split(',') 
        for i in key_list3:
            if i not in self.goods_dict:
                self.goods_dict[i] = 1
        self.thermometer_num = self.goods_dict['thermometer_num']
        if __name__=='__main__':
            print('温度计数量:', self.thermometer_num)
        ...
        ##################################################################### 特殊规则预处理
        cu_id = self.goods_dict['cu_id']
        cp_id = self.goods_dict['cp_id']
        goods_str = self.goods_dict['goods_str']

        try:
            goods_str_type_list = [x.split('==>')[1] for x in self.goods_dict.get('goods_str').split(';')]
        except:
            goods_str_type_list = []
            pass
        # print('tset1212', self.goods_dict.get('goods_str'), goods_str_type_list)
        goods_name = self.goods_dict['goods_name']
        rule_list = self.bin_special_rule_dict.get('rule_list')
        accept_rules_dict = {}
        find_rule_state = False

        for rule_i in rule_list:
            if1,if2,if3,if4 = False,False,False,False
            if rule_i["cu_id"] == "-1":
                if1 = True
            if rule_i["cp_id"] == "-1":
                if2 = True         
            if rule_i["goods_name"] == "-1":
                if3 = True
            if rule_i.get("goods_shape") == "-1":
                if4 = True      

            if not if1:
                if1 = cu_id == rule_i.get("cu_id")
            if not if2:
                if2 = cp_id == rule_i.get("cp_id")
            if not if3:
                if3 = self.check_str_list_in(goods_name, rule_i.get("goods_name").split('|'))
            if not if4:
                if4 = self.check_str_list_in(rule_i["goods_shape"], goods_str_type_list)
            # print('=========================================================================')
            # print('if1.............',if1, cu_id,rule_i.get("cu_id"))
            # print('if2.............',if2, cp_id,rule_i.get("cp_id"))
            # print('if3.............',if3)
            # print('if4.............',if4, rule_i["goods_shape"], goods_str_type_list)
            if_state = if1 and if2 and if3 and if4
            # if __name__=='__main__':
            #     print(rule_i)
            # print(if_state, if1 , if2 , if3 , if4, cu_id, rule_i.get("cu_id"), type(cu_id), type(rule_i.get("cu_id")))
            if if_state: # rule_i.get("cu_id").split('-') 
                find_rule_state = True
                break
        ########### 找到了特殊规则
        if find_rule_state:
            if __name__=='__main__':
                print('找到了特殊规则', rule_i)
            if rule_i['reshape_str']!='-1':  # 需要reshape
                pass
            if rule_i['min_bin']!='-1':  # 需要reshape
                all_bins_ascend_list = list(self.container_info['bin_成本'].keys())
                min_index = 1000
                for bin_x in rule_i['min_bin'].split('|'):
                    index_x = all_bins_ascend_list.index(bin_x)
                    if index_x < min_index:
                        min_index = index_x
                self.rule_accept_bins = all_bins_ascend_list[min_index:]
                # print('self.rule_accept_bins..........', self.rule_accept_bins)

            if rule_i['min_bin']=='-1':
                self.rule_accept_bins = list(self.container_info['bin_成本'].keys())

            if rule_i['min_margin_cm']!='-1':  # 需要reshape
                self.min_margin_cm = float(rule_i['min_margin_cm'])
                # print('..........self.min_margin_cm',self.min_margin_cm)
        else:
            self.rule_accept_bins = list(self.container_info['bin_成本'].keys())
        ...
        for k,v in self.final_goods_dict.items():
            ...
            self.shape_thermometer_num[goods_str] = min_n123456*self.thermometer_num
    ...
    def get_accept_bins(self):
        ...
        # self.rule_accept_bins
        # print('test1212', accept_bins_dict)
        self.accept_bins_dict = {}
        for k,v in accept_bins_dict.items():
            if k in self.rule_accept_bins:
                # print('test rule_accept_bins key', k, '==', v)
                self.accept_bins_dict[k] = v
    ...
    def one_goods_one_box_package(self, c_l,c_w,c_h,c_max_v, goods_qty,goods_l,goods_w,goods_h):  ###### 将一种物品放入一个container
        ...
        lwh_left_list1 = [round(x,2) for x in [l_final_left, ww_left, int(c_h-max_num*goods_h)]]  # 最终长，宽，有效高的结余量
        ...
        wl_method_n = int(max_num2*lw_layer_max_n)  # 长长放法的最大数量 

        # print(c_l,c_w,c_h, '长宽放法层数', max_num2, ll_layer_max_n,wl_method_n)
        lwh_left_list2 = [round(x,2) for x in [lw_left, w_final_left, int(c_h-max_num2*goods_h)]]  # 最终长，宽，有效高的结余量
        ...
        # print(c_l,c_w,c_h, '长长每层，长宽每层', ll_layer_max_n, lw_layer_max_n)
        return [ll_method_n,wl_method_n],[lwh_left_list1, lwh_left_list2],[ll_layer_max_n, lw_layer_max_n],[max_num,max_num2],dict_str  # 长长放法数量，和宽长方法数量,每层放的数量,层数
    ...
    def select_able_one_bin_one_item(self,goods_str='107==>12*8*2;'):
        ...
        for k,v in c_container_dict.items():
            ...
            list1,list2,list3,list4,dict_str = self.one_goods_one_box_package(c_l,c_w,c_h,c_max_v, goods_qty,goods_l,goods_w,goods_h)
    ...
    def solution_list(self):
        ...
        for k,v in self.final_goods_dict.items():
            ...
            df = df.append(df_i)
        if __name__=='__main__':
            try:df.to_excel('test_output/solution_list123.xlsx', index=False)
            except:pass        
        ...
        # df = df.sort_values('max_L', ascending=False)
        ...
        for i,r in df.iterrows():
            if max(r['l余量'], r['w余量'])>=lw_mmm_cm and min(r['l余量'], r['w余量'])>=self.min_margin_cm:
                new_df_rows.append(r)
            else:
                # print(dict(r))
                if (r['l余量']>0 or r['w余量']>0):
                    dict_str_i = json.loads(r['dict_str'])
                    total_num = r['每箱']*r['整箱']+r['last箱']
                    if r['放法']=='长长':
                        layer_i1 = int(3/r['w'])+1
                        sub_i1 = dict_str_i['长长数']*r['层数']*layer_i1

                        layer_i2 = int(3/r['l'])+1
                        sub_i2 = dict_str_i['宽宽数']*r['层数']*layer_i2
                        sub_i = min(sub_i2, sub_i1)  # 得到需要减去的数量

                        if sub_i1<sub_i2:
                            r['w余量'] = layer_i1*r['w']+r['w余量']
                        else:
                            r['l余量'] = layer_i2*r['l']+r['l余量']

                    else:
                        layer_i1 = int(3/r['w'])+1
                        sub_i = dict_str_i['宽长数']*r['层数']*layer_i1
                        r['l余量'] = layer_i1*r['l']+r['w余量']
                    try:
                        r['每箱'] = r['每箱'] - sub_i
                        r['每层'] = int(sub_i/r['层数'])
                        r['整箱'] = int(total_num/r['每箱'])
                        r['last箱'] = int(total_num%r['每箱'])
                        r['余量'] = r['每箱']-r['last箱']
                    except:
                        pass

                new_df_rows.append(r)
        df = pd.DataFrame(new_df_rows, columns=df.columns)
        # df['max_lw'] = max_lw_list
        # df['min_lw'] = min_lw_list
        # df0 = df[(df['min_lw']>self.min_margin_cm)&(df['max_lw']>3)].reset_index(drop=True)
        # df = 

        if __name__=='__main__':
            try:df.to_excel('test_output/has_solution_goods筛选后.xlsx', index=False)
            except:pass
        ...
        if len(has_solution_goods)==1:  ############ 只有一个货物时
            ...
            if r_dict.get('bb_num')>1:  # 一种类型货物，需要多个箱子
                # print(df)
                appear_list1 = []
                dfxx11_rows = []
                for xj,r in df.iterrows():
                    if r['箱子'] not in appear_list1:
                        appear_list1.append(r['箱子'])
                        dfxx11_rows.append(r)

                dfxx11 = pd.DataFrame(dfxx11_rows, columns=df.columns).reset_index(drop=True)
                if __name__=='__main__':
                    try:dfxx11.to_excel('test_output/dfxx11.xlsx', index=False)
                    except:pass
                ...
                first_bin_name = dfxx11['箱子'][0]
                # print('goods_num, 每箱', goods_num, dfxx11['每箱'][0])
                first_bin_num = int(goods_num//dfxx11['每箱'][0])
                mydict['res_dict'] = {first_bin_name:first_bin_num}
                # print('first_bin_name,first_bin_num', first_bin_name,first_bin_num)
                last_bin_num = goods_num-first_bin_num*dfxx11['每箱'][0]
                # print('first_bin_num,last_bin_num', first_bin_num,last_bin_num)

                df_x = dfxx11[dfxx11['每箱']>=last_bin_num]
                df_x=df_x.sort_values('max_L').reset_index(drop=True)
                # print(df_x)
                last_bin_name = df_x['箱子'][0]
                # print('last_bin_name', last_bin_name)
                if last_bin_name not in mydict['res_dict']:
                    mydict['res_dict'][last_bin_name] = 1
                else:
                    mydict['res_dict'][last_bin_name] +=1
                # print('mydict............', mydict)
                self.msg['求解方案说明'] = "单货物，放法：最大长长11"
                return mydict
        ...
        for bin_i in sort_cost_bins:
            ...
            for lay_type in df_i['放法'].unique().tolist():                         # 循环所有可行的放法
                ...
                if '长长' in lay_type:
                    ...
                    space_h = [
                        int(dict_str['长长数']*goods_str1_dict['l']),
                        int(n1*goods_str1_dict['w']),
            ...
            for space_name,space_v in space_dict.items():       # 取余空间的类别（每个类别有多组）
                ...
                for goods,g_dict in space_goods_dict.items():   # 取某一个货物
                    for sp_k,sp_v in space_v.items():       # 取某一个余空间
                        ...
                        # print('正常', goods_lt,goods_wt,goods_ht,goods_l,goods_w,goods_h)
                        list1,list2,list3,list4,dict_str = self.one_goods_one_box_package(c_l,c_w,c_h,c_max_v, goods_qty,goods_l_real,goods_w_real,goods_h_real)
    ...
    def run(self):
        ...
        if res.get('res_dict') is not None:
            ...
            res_dict['res_dict'] = str(res.get('res_dict'))
            res_dict['msg'] = self.msg
...
if __name__=='__main__':
    ...
    # 'omc_tem_id': '31',         # 温度id
    # 'can_reverse': 0,           # 是否可以倒放
    # 'goods_str': '3==>16.0*12.0*6.0',   # 货物尺寸20个6*6*11.5, ...
    # 'is_fragile': 0             # 是否是易碎品
    # }
    ...
    # 'omc_tem_id': '31',         # 温度id
    # 'can_reverse': 0,           # 是否可以倒放
    # 'goods_str': '1==>1*1*3;',   # 货物尺寸20个6*6*11.5, ...
    # 'is_fragile': 0             # 是否是易碎品
    # }

    # goods_dict = {'is_fragile': '0',
    #  'can_reverse': '1', 
    #  'start_region_id': '110000', 'end_region_id': '110000', 
    # 'route_type': '-1', 'omc_tem_id': '31', 'goods_str': '30==>15.00*16.00*17.00', 
    # 'specail_type_str': None, 'omc_time': '2022-03-14', 'cp_id': '2920', 'cg_name': '蓝冰样本一', 'cu_id': '201600962'}
    

    ################################## 测试数量变多，箱子反而变少
    # goods_dict = {"is_fragile":"0","can_reverse":"1",
    # "start_region_id":"110000","end_region_id":"110000",
    # "route_type":"-1","omc_tem_id":"31",
    # "goods_str":"30==>15.00*16.00*17.00","specail_type_str":"","omc_time":"2022-03-14",
    # "cp_id":"2920","cg_name":"-1","cu_id":"201600962"}

    # goods_dict = {"is_fragile":"0","can_reverse":"1",
    # "start_region_id":"110000","end_region_id":"110000",
    # "route_type":"-1","omc_tem_id":"31",
    # "goods_str":"40==>15.00*16.00*17.00","specail_type_str":"","omc_time":"2022-03-14",
    # "cp_id":"2920","cg_name":"-1","cu_id":"201600962"}

    # # ################################## 测试xxxx
    # goods_dict =  {"is_fragile":"0","can_reverse":"1","start_region_id":"110000",
    # "end_region_id":"110000","route_type":"-1","omc_tem_id":"31",
    # "goods_str":"10==>15.00*16.00*17.00;10==>5.00*3.00*6.00","specail_type_str":"",
    # "omc_time":"2022-03-14","cp_id":"2920","cg_name":"xxx","cu_id":"201600962"}
    ...
    返回：'VIP168*2,VIP3*3'
    '''
    res = OneContainerOneItemSolution(goods_dict).run()
    if __name__=='__main__':
        import myutils.dict_json_saver as mdjs
        mdjs.save_dict_to_json(res, file_path='test_output/结果test.json')
        pass
